...
flowchart TB
   ...
   B -->|合约| C1[合约规则<br/>LTV=75% + events + 安全组件]
   B -->|前端| C2[读写分离<br/>provider读 / signer写]
   B -->|交易可靠性| C3[tx状态机<br/>pending/stuck/replacement]
   B -->|UI更新| C4[事件驱动刷新<br/>+ throttle + backfill]
   B -->|测试/部署| C5[Hardhat测试<br/>+ deploy脚本导出ABI/地址]

   C1 --> E1[证据点<br/>contracts/SimpleLending.sol]
   C2 --> E2[证据点<br/>useWallet/useDashboard/useActions]
   C3 --> E3[证据点<br/>frontend/src/state/tx.ts]
   C4 --> E4[证据点<br/>frontend/src/hooks/useDashboard.ts]
   C5 --> E5[证据点<br/>scripts/deploy.ts + test/*]
```
