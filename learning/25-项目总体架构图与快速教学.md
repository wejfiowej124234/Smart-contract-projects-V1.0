...
相关文档（强关联）：
- 题面原文：[docs/archive/CODING_TEST_ASSIGNMENT.txt](../docs/archive/CODING_TEST_ASSIGNMENT.txt)
- 需求映射表：[docs/archive/ASSESSMENT_MAPPING.md](../docs/archive/ASSESSMENT_MAPPING.md)
- 安全说明：[SECURITY.md](../SECURITY.md)
- 审计式自查：[docs/archive/ENTERPRISE_PROJECT_AUDIT_企业级项目审计.md](../docs/archive/ENTERPRISE_PROJECT_AUDIT_企业级项目审计.md)（短版见 [docs/archive/ENTERPRISE_AUDIT_REPORT.md](../docs/archive/ENTERPRISE_AUDIT_REPORT.md)）
...
（可点击导航）
- 对齐项清单：[docs/archive/ASSESSMENT_MAPPING.md](../docs/archive/ASSESSMENT_MAPPING.md)
...
flowchart TB
  ...
  subgraph HARDHAT[Hardhat 工程（仓库根目录）]
    C[contracts/<br/>SimpleLending.sol<br/>TestToken.sol]
    S[scripts/<br/>deploy.ts + _lib/export.ts]
    A[artifacts/ & cache/<br/>编译产物]
    D[deployments/<br/>31337.json]
    T[test/<br/>*.integration.ts]
  end

  CLI -->|compile| A
  CLI -->|hardhat node| CHAIN[本地链 31337<br/>http://127.0.0.1:8545]
  CLI -->|run scripts/deploy.ts| S
  S -->|deploy + seed| CHAIN
  S -->|write json| D
  S -->|export ABI + addresses| FE_ART[frontend/src/abis/*.json<br/>frontend/src/contracts/deployments.json]

  subgraph FE[前端（frontend/）]
    UI[App.tsx<br/>页面/UI]
    WAL[hooks/useWallet<br/>连接钱包/切链/持久化]
    READ[hooks/useDashboard<br/>读链/事件刷新]
    WRITE[hooks/useActions<br/>写链/approve 流]
    TX[state/tx.ts<br/>交易状态机]
    ERR[state/errors.ts<br/>错误归一化]
  end
  ...
  MM[MetaMask<br/>浏览器钱包] <-->|授权/签名| WAL
  DEV -->|打开网页| UI
```
...
强烈建议你在演示/面试前，按这份 runbook 走一遍：[docs/DEMO_CHECKLIST.zh-en.md](../docs/archive/DEMO_CHECKLIST.zh-en.md)。
...
flowchart TB
  ...
  N --> K{kind}
  K --> UR[UserRejected<br/>4001]
  K --> RV[Revert<br/>CALL_EXCEPTION/reverted]
  K --> RPC[Rpc<br/>network/timeout]
  K --> UNK[Unknown]
```
...
- 审计式自查报告：[docs/archive/ENTERPRISE_PROJECT_AUDIT_企业级项目审计.md](../docs/archive/ENTERPRISE_PROJECT_AUDIT_企业级项目审计.md)（短版 [docs/archive/ENTERPRISE_AUDIT_REPORT.md](../docs/archive/ENTERPRISE_AUDIT_REPORT.md)，强调不是第三方审计）
...
%%{init: {'theme': 'dark'}}%%
flowchart TB
  UI[App.tsx<br/>Borrow button] --> PF[Preflight 确认]
...
%%{init: {'theme': 'dark'}}%%
flowchart TB
  UI[App.tsx<br/>Withdraw button] --> ACT[useActions.withdraw]
  ACT --> VAL[requireAmountStrict]
  VAL --> PRE[前端预检查:<br/>usd8.balanceOf(lending) >= amount]
  PRE -->|No| ERR[throw "Insufficient liquidity"<br/>(不弹钱包)]
  PRE -->|Yes| TX[runTxDetailed]
  TX --> L[lending.withdraw(amount)]
  L --> HF[合约检查:<br/>Withdrawal would make position unhealthy]
...
%%{init: {'theme': 'dark'}}%%
flowchart TB
  TOP[Top bar<br/>Connect / Refresh<br/>ChainId / Account] --> ADDR[Contract addresses<br/>USD8/WETH/Lending]
  ADDR --> ERR[Errors<br/>Wallet error / Dashboard error]
  ERR --> NET[Wrong network banner<br/>Switch to 31337]
  NET --> TX[Tx status line<br/>+ stuck helper panel]
  TX --> PF[Preflight modal<br/>(pre-wallet confirmation)]
  PF --> ACT[Actions<br/>Approve mode + 4 cards]
  ACT --> DATA[Balances / Pool / User Position<br/>+ Max(safe) helpers]
```
...
- 证据：[docs/archive/CODING_TEST_ASSIGNMENT.txt](../docs/archive/CODING_TEST_ASSIGNMENT.txt)、[docs/archive/ASSESSMENT_MAPPING.md](../docs/archive/ASSESSMENT_MAPPING.md)、[docs/archive/DEMO_CHECKLIST.zh-en.md](../docs/archive/DEMO_CHECKLIST.zh-en.md)
